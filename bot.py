...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Hey! I'm the B&FS Term V timetable bot. 👋\n\n"
...
async def codes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin-only diagnostic: lists every subject code the parser is
    currently picking up from the live sheet, split into ones that ARE in
    SUBJECTS (selectable) vs ones that aren't. Catches a new/renamed code,
    a term-transition mixup, or a subject silently falling through as an
    unrecognized event — the kind of thing that's easy to miss otherwise."""
    chat_id = update.effective_chat.id
    ...
    counts = {}
    for day_block in SCHEDULE.values():
        for sess in day_block["sessions"].values():
            for cls in sess["classes"]:
                code = cls["canonical"]
                counts[code] = counts.get(code, 0) + 1

    known = sorted((c, n) for c, n in counts.items() if c in SUBJECTS)
    unknown = sorted((c, n) for c, n in counts.items() if c not in SUBJECTS)

    lines = ["🔎 *Codes currently detected in the live sheet:*\n"]
    lines.append("*In your SUBJECTS list:*")
    for c, n in known:
        lines.append(f"  • {c}: {n} sessions")
    lines.append("\n*NOT in your SUBJECTS list* (other sections, or a code you might be missing):")
    if unknown:
        for c, n in unknown:
            lines.append(f"  • {c}: {n} sessions")
    else:
...
async def revokeall(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin-only: wipes every approval and pending request, forcing
    everyone (except the admin) to re-request access from scratch on their
    next message. Does NOT touch anyone's saved subjects/reminders — only
    ...
    if not ADMIN_CHAT_ID or chat_id != ADMIN_CHAT_ID:
        ...
        return
    conn = _approval_db()
    approved_count = conn.execute("SELECT COUNT(*) FROM approved_users").fetchone()[0]
    pending_count = conn.execute("SELECT COUNT(*) FROM pending_requests").fetchone()[0]
    conn.execute("DELETE FROM approved_users")
    conn.execute("DELETE FROM pending_requests")
    conn.commit()
    conn.close()
    await update.message.reply_text(
        f"✅ Revoked {approved_count} approval(s) and cleared {pending_count} "
        f"pending request(s). Everyone (except you) will need to /start again "
        f"and wait for a fresh approval."
    )
...
def main():
    ...
    app.add_handler(CommandHandler("revokeall", revokeall))
